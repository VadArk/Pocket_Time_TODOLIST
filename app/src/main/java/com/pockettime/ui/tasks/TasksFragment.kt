...
import com.pockettime.databinding.FragmentTasksBinding
import com.pockettime.util.exhaustive
import com.pockettime.util.onQueryTextChanged
...
class TasksFragment : Fragment(R.layout.fragment_tasks), TasksAdapter.OnItemClickListener {
        ...
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            viewModel.tasksEvent.collect { event ->
                when (event) {
                    ...
                    is TasksViewModel.TasksEvent.ShowTaskSavedConfirmMess -> {
                        Snackbar.make(requireView(), event.msg, Snackbar.LENGTH_LONG).show()
                    }
                    TasksViewModel.TasksEvent.NavigateToDeleteAllCompletedScreen -> {
                        val action =
                            TasksFragmentDirections.actionGlobalDeleteAllCompletedDialogFragment3()
                        findNavController().navigate(action)
                    }
                }.exhaustive
            }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        val searchItem = menu.findItem(R.id.search_bottom)
